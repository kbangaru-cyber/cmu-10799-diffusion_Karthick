...
Usage:
    ...
    # Sample from Flow Matching with Heun solver (recommended, more accurate)
    python sample.py --checkpoint checkpoints/flow_matching_final.pt --method flow_matching --num_steps 200 --solver heun --grid
"""
...
def main():
    ...
    # NEW: Flow matching specific options
    parser.add_argument(
        "--solver",
        type=str,
        default="heun",
        choices=["euler", "heun"],
        help="ODE solver for flow matching (heun is more accurate, recommended)"
    )
    parser.add_argument(
        "--no_clamp",
        action="store_true",
        help="Don't clamp final output to [-1, 1] (usually not recommended)"
    )
    ...
    if args.method == "flow_matching":
        print(f"Generating {args.num_samples} samples with num_steps={num_steps}, solver={args.solver}...")
    else:
    ...
    with torch.no_grad():
        ...
        while remaining > 0:
            ...
            if args.method == "flow_matching":
                # Use the new solver and clamp options for flow matching
                try:
                    samples = method.sample(
                        ...
                        num_steps=num_steps,
                        solver=args.solver,
                        clamp=not args.no_clamp,  # Clamp final output by default
                    )
                except TypeError:
                    # Fallback for older versions without solver parameter
